...
Begin
  Repeat
    ClrScr;
    Write('Hur många rader/kolonner skall behandlas :');
    ReadLn(nelm);

    nolla_mat(ma,nelm);
    nolla_mat(mb,nelm);

    WriteLn('Inläsning av den första matrisen.');
    las_in_mat(ma,nelm);

    WriteLn('Inläsning av den andra matrisen.');
    las_in_mat(mb,nelm);
    ...
    WriteLn('Den första matrisen:');
    skriv_ut_mat(ma,nelm);

    WriteLn('Den andra matrisen:');
    skriv_ut_mat(mb,nelm);

    Writeln('Resultatet av multiplikation med de två inmatade matriserna ovan.');
    skriv_ut_mat(mc,nelm);

    WriteLn('Vill du göra det en gång till tryck "J"');
    Read(Kbd,tkn);
