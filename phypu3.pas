...
Begin
  ClrScr;
  WriteLn('Värden inom [] är default');
  WriteLn;
  Write('Indelningar vid plottning [',indelningar,']:');
  ReadLn(indelningar);
  WriteLn;
  WriteLn('Koordinater vid plottning');
  Write('Nedre vänstra hörnet [',xmin:8:2,' ',ymin:8:2,']:');
  ReadLn(xmin,ymin);
  Write('Övre högra hörnet [',xmax:8:2,' ',ymax:8:2,']:');
  ReadLn(xmax,ymax);
  WriteLn;
  WriteLn('Koordinater på plottern');
  Write('Nedre vänstra hörnet [',pxmin:8,' ',pymin:8,']:');
  ReadLn(pxmin,pymin);
  Write('Övre högra hörnet [',pxmax:8,' ',pymax:8,']:');
  ReadLn(pxmax,pymax);
  ClrScr;
End;
